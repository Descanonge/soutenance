...
\begin{frame}[label=chlsnapshot]
  ...
  \begin{beamercolorbox}[sep=0pt, right]{}
    ...
    \\
    {\footnotesize Chlorophylle-a L2 MODIS-Terra, 20 février 2020}
  \end{beamercolorbox}
\end{frame}
...
\begin{frame}
  ...
  \includegraphics[height=0.8\textheight]{_res_perm/med_base<1->.pdf}%
  \graphstep{xlabels_med}{1-4}%
  \graphstep{xlabels_exces}{4}%
  \graphstep{2020}{1}%
  \graphstep{all_years}{2}%
  \graphstep{climato}{3-}%
  \graphstep{med_base_top}{1-}%
  \graphstep{exces}{4-}%
  \graphstep{surface}{5-}%
  \graphstep{surplus}{6-}%
    \end{column}
    \begin{column}{0.45\textwidth}
      ...
      \begin{itemize}
        \setlength{\itemsep}{5em}
              \visible<4->{\item Excès :\\local, avec la médiane
              \\
              \[E = \frac {\text{fronts} - \text{background}} {\text{background}} \]}
              \visible<6->{\item Surplus :\\pour la région, avec la \emph{moyenne}
              \\
              \[S = \frac {\text{total} - \text{background}} {\text{background}} \]}
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
